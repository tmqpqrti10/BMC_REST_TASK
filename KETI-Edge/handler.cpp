...
void Handler::handle_patch(http_request _request)
{
    ...
    //         Collection *roled_collection =(Collection *)g_record[ODATA_ROLE_ID];
    //         json::value obj = account_collection->get_json();
    ...
    //                 if (role_id !="")
    //                 {
    //                     vector<Resource *> *roles= roled_collection->get_member();
    ...
    //             strcpy(((char *)chassis->fru_this->product.asset_tag),c_assettag.c_str());
    ...
    //             strcpy(((char *)chassis->fru_this->fru_board.mfg),c_mfg.c_str());
    ...
    //             strcpy(((char *)chassis->fru_this->fru_chassis.serial),c_serial.c_str());
    ...
    //             strcpy(((char *)chassis->fru_this->fru_chassis.part_number),c_serial.c_str());
    ...
    //             strcpy(((char *)systems->fru_this->product.asset_tag),s_assettag.c_str());
...
void Handler::handle_post(http_request _request)
{
    ...
    try
    {
        ...
        //     auto it = find(systems->actions.actions.begin(), systems->actions.actions.end(), action);
        //     if (it == systems->actions.actions.end())
        //     {
        ...
        //         }
        //         else if(action=="Off"||action=="ForceOff"||action =="GracefulShutdown")
        //         {
        //             //미구현상태
        //             redfish_power_ctrl=0;
        //         }
        //         else if(action=="ForceRestart"||action=="Reset"||action =="GracefulRestart")
        //         {
        //             //미구현상태
        //             redfish_power_ctrl=1;
        //         }
...
//     _response.headers().add("Access-Control-Allow-Methods", "POST, GET, OPTIONS, DELETE, PATCH");
//     _response.headers().add("Access-Control-Max-Age", "3600");
//     _response.headers().add("Access-Control-Allow-Headers", "Content-Type, Accept, X-Requested-With, remember-me, X-Auth-Token");
//     _response.headers().add("Access-Control-Expose-Headers", "X-Auth-Token, Location");
...
//     // task리소스 마무리부분 넣고 response생성해서 return; cors도 여기서 집어넣자
// }
...
void treat_post(http_request _request, json::value _jv, http_response& _response)
{
    ...
    error_reply(get_error_json("URI Input Error in treat POST"), status_codes::NotImplemented, _response);
    return ;
}
...
void do_actions(http_request _request, json::value _jv, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("URI Input Error in Resource part"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    {
        error_reply(get_error_json("URI Input Error in action_by check"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    error_reply(get_error_json("오면안되는곳"), status_codes::BadRequest, _response);
    return ;
}
...
void act_certificate(json::value _jv, string _resource, string _what, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Action in Certificate"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    if(_what == "Rekey")
    {
        ...
        {
            error_reply(get_error_json("Request Body error in Certificate Rekey"), status_codes::BadRequest, _response);
            return ;
        }
        else
        {
    ...
    else if(_what == "Renew")
    {
        ...
        if(result == json::value::null())
        {
            error_reply(get_error_json("No Files Error in Certificate ReNew"), status_codes::BadRequest, _response);
            return ;
        }
        else
        {
...
void act_certificate_service(json::value _jv, string _resource, string _what, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Action in Certificate Service"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    else if(_what == "ReplaceCertificate")
    {
        ...
        {
            error_reply(get_error_json("Problem occur in ReplaceCertificate()"), status_codes::BadRequest, _response);
            return ;
        }
...
void act_system(json::value _jv, string _resource, string _what, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Action in ComputerSystem"), status_codes::BadRequest, _response);
        return ;
    }

    if(!(system->Reset(_jv)))
    {
        error_reply(get_error_json("Problem occur in Reset()"), status_codes::BadRequest, _response);
        return ;
    }
...
void act_eventservice(json::value _jv, string _resource, string _what, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Action in EventService"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    if(result == json::value::null())
    {
        error_reply(get_error_json("Need Correct Request Body"), status_codes::BadRequest, _response);
        return ;
    }
...
void act_logservice(json::value _jv, string _resource, string _what, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Action in LogService"), status_codes::BadRequest, _response);
        return ;
    }

    if(!(log_service->ClearLog()))
    {
        error_reply(get_error_json("Problem occur in ClearLog()"), status_codes::BadRequest, _response);
        return ;
    }
...
void act_virtualmedia(json::value _jv, string _resource, string _what, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Action in VirtualMedia"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    if(result_value == json::value::null())
    {
        ...
        error_reply(get_error_json("Problem Occur in VirtualMedia Actions"), status_codes::BadRequest, _response);
        return ;
    }
    else
    {
...
void make_account(json::value _jv, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("Account Service isn't Enabled"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    {
        error_reply(get_error_json("No UserName or Password"), status_codes::BadRequest, _response);
        return ;
    ...
    {
        error_reply(get_error_json("Password length must be at least " + to_string(min_pass_length)), status_codes::BadRequest, _response);
        return ;
    }// password 길이가 짧으면 BadRequest

    if(password.size() > max_pass_length)
    {
        error_reply(get_error_json("Password length must not exceed " + to_string(max_pass_length)), status_codes::BadRequest, _response);
        return ;
    ...
    for(iter=col->members.begin(); iter!=col->members.end(); iter++)
    {
        ...
        {
            error_reply(get_error_json("UserName already exists"), status_codes::Conflict, _response);
            return ;
        }
...
void make_session(json::value _jv, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("Session Service isn't Enabled"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    if(!exist)
    {
        error_reply(get_error_json("No Account using that UserName"), status_codes::BadRequest, _response);
        return ;
    } // 입력한 username에 해당하는 계정없음

    if(account->password != password)
    {
        error_reply(get_error_json("Password does not match"), status_codes::BadRequest, _response);
        return ;
    } // 비밀번호 맞지않음
...
void treat_patch(http_request _request, json::value _jv, http_response& _response)
{
    ...
    if(uri_part == ODATA_ACCOUNT_SERVICE_ID)
    {
        ...
        if(uri == ODATA_ACCOUNT_SERVICE_ID)
        {
            ...
            {
                error_reply(get_error_json("Error Occur in AccountService PATCH"), status_codes::BadRequest, _response);
                return ;
            }
        }
    ...
    else if(uri_part == ODATA_SESSION_SERVICE_ID)
    {
        ...
        if(uri == ODATA_SESSION_SERVICE_ID)
        {
            ...
            {
                error_reply(get_error_json("Error Occur in SessionService PATCH"), status_codes::BadRequest, _response);
                return ;
            }
        }
    }
    ...
    else if(uri_part == ODATA_MANAGER_ID)
    {
        ...
        if(uri == bmc_manager)
        {
            if(patch_manager(_jv, uri))
            {
                ...
                error_reply(get_error_json("Error Occur in Manager PATCH"), status_codes::BadRequest, _response);
            return ;
        }
        ...
        if(uri == bmc_manager + "/NetworkProtocol")
        {
            ...
            {
                error_reply(get_error_json("No NetworkProtocol"), status_codes::BadRequest, _response);
                return ;
            }
            ...
            {
                error_reply(get_error_json("Error Occur in NetworkProtocol PATCH"), status_codes::BadRequest, _response);
                return ;
            }
        }
        ...
        if(uri == bmc_manager + "/FanMode")
        {
            ...
            {
                error_reply(get_error_json("Json Body is Null"), status_codes::BadRequest, _response);
                return ;
            }
            ...
            if(get_value_from_json_key(_jv, "Mode", mode))
            {
                ...
                {
                    error_reply(get_error_json("Incorrect Mode"), status_codes::BadRequest, _response);
                    return ;
                }
            }
            else
            {
                error_reply(get_error_json("No Mode in Json Body"), status_codes::BadRequest, _response);
                return ;
            }
        ...
        if(minus_one == bmc_manager + "/EthernetInterfaces")
        {
            ...
            {
                error_reply(get_error_json("No EthernetInterface"), status_codes::BadRequest, _response);
                return ;
            }
            ...
            {
                error_reply(get_error_json("Error Occur in EthernetInterface PATCH"), status_codes::BadRequest, _response);
                return ;
            }
        }
    ...
    else if(uri_part == ODATA_SYSTEM_ID)
    {
        ...
        if(uri == bmc_system)
        {
            if(patch_system(_jv, uri))
            {
                ...
                error_reply(get_error_json("Error Occur in System PATCH"), status_codes::BadRequest, _response);
            return ;
        }
    }
    ...
    else if(uri_part == ODATA_CHASSIS_ID)
    {
        ...
        if(uri == bmc_chassis)
        {
            if(patch_chassis(_jv, uri))
            {
                ...
                error_reply(get_error_json("Error Occur in Chassis PATCH"), status_codes::BadRequest, _response);
            return ;
        }
        ...
        if(minus_one == bmc_chassis + "/Power/PowerControl")
        {
            if(!record_is_exist(uri))
            {
                error_reply(get_error_json("No PowerControl"), status_codes::BadRequest, _response);
                return ;
            }
            ...
            if(patch_power_control(_jv, uri))
            {
                ...
                error_reply(get_error_json("Error Occur in PowerControl PATCH"), status_codes::BadRequest, _response);
            return ;
        }
    }
    ...
    else if(uri_part == ODATA_EVENT_SERVICE_ID)
    {
        if(uri == ODATA_EVENT_SERVICE_ID)
        {
            if(patch_event_service(_jv, uri))
                ...
                error_reply(get_error_json("Error Occur in EventService PATCH"), status_codes::BadRequest, _response);
        ...
        if(minus_one == ODATA_EVENT_DESTINATION_ID)
        {
            ...
            if(!record_is_exist(uri))
            {
                error_reply(get_error_json("No Subscription"), status_codes::BadRequest, _response);
                return ;
            }
            ...
            if(patch_subscription(_jv, uri))
                ...
                error_reply(get_error_json("Error Occur in Subscription PATCH"), status_codes::BadRequest, _response);
    ...
    {
        error_reply(get_error_json("URI Input Error in Whole part"), status_codes::BadRequest, _response);
        return ;
    }

    error_reply(get_error_json("URI Input Error in Below part"), status_codes::BadRequest, _response);
    return ;
}
...
void modify_account(http_request _request, json::value _jv, string _uri, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("No Account"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    if(!_request.headers().has("X-Auth-Token"))
    {
        error_reply(get_error_json("Login Required"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    {
        error_reply(get_error_json("Session Unvalid"), status_codes::BadRequest, _response);
        return ;
    }
    ...
    if(session->account->role->id == "Administrator")
    {
        ...
        if(get_value_from_json_key(_jv, "RoleId", role_id))
        {
            ...
            {
                error_reply(get_error_json(role_id + " does not exist"), status_codes::BadRequest, _response);
                return ;
            }
            op_role = true;
        }
    ...
    if(session->account->role->id == "Administrator" || session->account->id == last)
    {
        ...
        if(get_value_from_json_key(_jv, "UserName", input_username))
        {
            ...
            for(iter=col->members.begin(); iter!=col->members.end(); iter++)
            {
                ...
                {
                    error_reply(get_error_json("UserName is already in use"), status_codes::BadRequest, _response);
                    return ;
                }
        ...
        if(get_value_from_json_key(_jv, "Password", input_password))
        {
            ...
            {
                error_reply(get_error_json("Password Range is " + to_string(min) + " ~ " + to_string(max)), status_codes::BadRequest, _response);
                return ;
            } // password 길이 검사
            op_pass = true;   
        }
    ...
    {
        error_reply(get_error_json("Cannot Modify Account"), status_codes::BadRequest, _response);
        return ;
    }
    if(user==NULL)
    {
...
void modify_role(http_request _request, json::value _jv, string _uri, http_response& _response)
{
    ...
    if(login_role->id == "Administrator")
    {
        ...
        {
            error_reply(get_error_json("No Privileges"), status_codes::BadRequest, _response);
            return ;
        }
        ...
        for(int i=0; i<arr.size(); i++)
        {
            ...
            {
                //올바른 privilege 가 아님!!
                error_reply(get_error_json("Incorrect Previlege"), status_codes::BadRequest, _response);
                return ;
            }
        }
    ...
    {
        error_reply(get_error_json("Only Administrator Can Use"), status_codes::BadRequest, _response);
        return ;
    }
...
void treat_delete(http_request _request, json::value _jv, http_response& _response)
{
    ...
    error_reply(get_error_json("URI Input Error in treat delete"), status_codes::BadRequest, _response);
    return ;
}
...
void remove_account(json::value _jv, string _uri, string _service_uri, http_response& _response)
{
    ...
    // || get_value_from_json_key(_jv, "Password", password) == false)
    // {
    //     reply_error(_request, get_error_json("No UserName or Password"), status_codes::BadRequest, _response);
    //     return ;
    // } // json request body에 UserName, Password없으면 BadRequest
    // ## 방식변경후 주석
    ...
    // if(!exist)
    // {
    //     reply_error(_request, get_error_json("No Account or Does not match Password"), status_codes::BadRequest, _response);
    //     return ;
    // } // ## 방식변경후 주석
    ...
    success_reply(record_get_json(acc_service->account_collection->odata.id), status_codes::OK, _response);
    return ;
}
...
void remove_session(http_request _request, http_response& _response)
{
    ...
    {
        error_reply(get_error_json("Unvalid Session"), status_codes::BadRequest, _response);
        return ;
    }
