...
void init_system(Systems *system, string _id)
{
    ...
    system->model = "";
    system->serial_number = std::string((char *)system->fru_this->fru_chassis.serial);
    system->part_number = std::string((char *)system->fru_this->fru_chassis.part_number);
    system->description = "";
    ...
    if (ipmiChassis.g_chassis_bootp.boot_flags.size() > 0)
    {
        ...
        if (enable == 1)
            system->boot.boot_source_override_enabled = (ipmiChassis.g_chassis_bootp.boot_flags.at(0) & BF1_VALID & BF1_PERSIST) ? "Continuous" : "Once";
        else
            system->boot.boot_source_override_enabled = "Disabled";
            
        switch (ipmiChassis.g_chassis_bootp.boot_flags.at(1) & BF1_VALID & BF2_BOOTDEV_MASK)
        {
...
void init_storage(Storage *storage)
{
    ...
    if (!record_is_exist(odata_id + "/StorageControllers")){
        ...
        int storage_num = improved_stoi(get_popen_string("lsblk -o NAME -n | wc -l"));
...
void init_storage_controller(List *storage_controllers_list, string _id)
{
    ...
    sc->manufacturer = get_popen_string("lsblk "+ block_dev +" -o NAME,VENDOR -n | awk \'{print $2}\'");
    sc->model = get_popen_string("lsblk "+ block_dev +" -o NAME,MODEL -n | awk \'{print $2}\'");
    sc->serial_number = get_popen_string("lsblk "+ block_dev +" -o NAME,SERIAL -n | awk \'{print $2}\'");
    sc->part_number = get_popen_string("lsblk "+ block_dev +" -o NAME,PARTUUID -n | awk \'{print $2}\'");
    sc->speed_gbps = 0; // 
    sc->firmware_version = ""; //
...
void init_processor(Collection *processor_collection, string _id)
{
    ...
    processor->max_speed_mhz = improved_stof(get_value_from_cmd_str("lscpu | grep \"BogoMIPS\" | head -1", "BogoMIPS"));
    processor->total_cores = improved_stoi(get_value_from_cmd_str("lscpu | grep \"CPU(s)\" | head -1", "CPU(s)"));
    processor->total_threads = improved_stoi(get_value_from_cmd_str("lscpu | grep \"Thread(s) per core\" | head -1", "Thread(s) per core")) * processor->total_cores;
...
void init_memory(Collection *memory_collection, string _id)
{
    ...
    memory->id = _id;
    // memory->rank_count = 2;
    memory->capacity_kib = improved_stoi(get_value_from_cmd_str("free -k", "Mem"));
...
void init_ethernet(Collection *ethernet_collection, string _id)
{
    ...
    ethernet->permanent_mac_address = get_popen_string("more /sys/class/net/" + eth_id + "/address");
    ethernet->mac_address = ethernet->permanent_mac_address;
    ethernet->speed_Mbps = improved_stoi(get_popen_string("more /sys/class/net/" + eth_id + "/speed"));
    ...
    ethernet->mtu_size = improved_stoi(get_popen_string("more /sys/class/net/" + eth_id + "/mtu"));
    ...
    // ethernet->name_servers = string_split(get_popen_string("more /etc/resolv.conf"), ' ');
    ...
    if (ethernet->link_status == "LinkUp"){
        int ipv4_num = improved_stoi(get_popen_string("ifconfig -a | grep eth0 | wc -l"));
        ...
        if (fs::exists(VLAN_CONF)){
            if (improved_stoi(get_popen_string("cat /proc/net/vlan/config | grep " + eth_id +" | wc -l"))){
                Vlan v;
                v.vlan_enable = true;
                v.vlan_id = improved_stoi(string_split(get_popen_string("cat /proc/net/vlan/config | grep " + eth_id), '|')[1]);
            }   
        }
    }
    ...
    ethernet_collection->add_member(ethernet);
        ...
        // sprintf((char *)macaddress.c_str(), "%02x.%02x.%02x.%02x.%02x.%02x\0", iptemp.at(0), iptemp.at(1), iptemp.at(3), iptemp.at(4), iptemp.at(5));
        ...
        // address.resize(30);
        // sprintf((char *)address.c_str(), "%d.%d.%d.%d\0", iptemp.at(0), iptemp.at(1), iptemp.at(2), iptemp.at(3));
        // address.resize(address.find('\u0000'));
        // //cout<<"Ethernet addr = "<<address<<endl;

        // iptemp=ipmiNetwork[index].net_mask;
        // subnetMask.resize(30);
        // sprintf((char *)subnetMask.c_str(), "%d.%d.%d.%d\0", iptemp.at(0), iptemp.at(1), iptemp.at(2), iptemp.at(3));
        ...
        // gateway.resize(30);
        // sprintf((char *)gateway.c_str(), "%d.%d.%d.%d\0", iptemp.at(0), iptemp.at(1), iptemp.at(2), iptemp.at(3));
...
void init_simple_storage(Collection *simple_storage_collection, string _id)
{
    ...
    for (auto str : vec){
        ...
        info.manufacturer = get_popen_string("lsblk "+ block_dev +" -o NAME,VENDOR -n | awk \'{print $2}\'");
        info.model = get_popen_string("lsblk "+ block_dev +" -o NAME,MODEL -n | awk \'{print $2}\'");
...
void init_chassis(Chassis *chassis, string _id)
{
    ...
    chassis->part_number = string((char *)chassis->fru_this->fru_chassis.part_number);
...
void init_account_service(AccountService *account_service)
{
    ...
    if (!record_is_exist(odata_id + "/Accounts")){
        ...
        for (int i = 0; i < get_user_cnt(); i++){
            ...
            string roled;
            string account_odata_id = account_service->account_collection->odata.id + "/";
            ...
            switch (user->getUserPriv()){
                case 0x1: roled ="Callback"; break;
                case 0x2: roled ="User"; break;
                case 0x3: roled ="Operator"; break;
                case 0x4: roled ="Administrator"; break;
                case 0xf: roled ="Noaccess"; break;
                default: roled ="Unknown"; break;
            }
            ...
            account->certificates = new Collection(certificate_collection_id, ODATA_CERTIFICATE_COLLECTION_TYPE);
