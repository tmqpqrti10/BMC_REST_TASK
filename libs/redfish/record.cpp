        ...
        switch (it->second->type)
        {
            ...
            case SENSOR_TYPE:
            {
                ...
                else
                {
                    ...
                    {
                        p_sdr->uc_thresh = sdr_convert_sensor_value_to_raw(p_sdr, se->thresh.upper_critical.reading);
                        printf("\t redfish %s uc updated uc %d\n",p_sdr->str,(int)p_sdr->uc_thresh);
                    }
                    if (p_sdr->unc_thresh != THRESH_NOT_AVAILABLE)
                    {
                        p_sdr->unc_thresh = sdr_convert_sensor_value_to_raw(p_sdr, se->thresh.upper_caution.reading);
                        // printf("\t redfish updated\n");
                    }
                    if (p_sdr->unr_thresh != THRESH_NOT_AVAILABLE)
                    {
                        p_sdr->unr_thresh = sdr_convert_sensor_value_to_raw(p_sdr, se->thresh.upper_fatal.reading);
                        // printf("\t redfish updated\n");
                    }
                    if (p_sdr->lc_thresh != THRESH_NOT_AVAILABLE)
                    {
                        p_sdr->lc_thresh = sdr_convert_sensor_value_to_raw(p_sdr, se->thresh.lower_critical.reading);
                        // printf("\t redfish updated\n");
                    }
                    if (p_sdr->lnc_thresh != THRESH_NOT_AVAILABLE)
                    {
                        p_sdr->lnc_thresh = sdr_convert_sensor_value_to_raw(p_sdr, se->thresh.lower_caution.reading);
                        // printf("\t redfish lnc updated\n");
                    }
                    if (p_sdr->lnr_thresh != THRESH_NOT_AVAILABLE)
                    {
                        p_sdr->lnr_thresh = sdr_convert_sensor_value_to_raw(p_sdr, se->thresh.lower_fatal.reading);
            ...
            case ACCOUNT_TYPE:{
                ...
                Ipmiuser *user=&ipmiUser[stoi(get_current_object_name(this_odata_id,"/"))];
...
void resource_save_json(Resource *Rsrc)
{
    ...
    if (record_is_exist(Rsrc->odata.id)){
        Rsrc->save_json();
        log(debug) << "[Resource Save Json] : " << this_odata_id << " is succeesfully saved..";
    } else {
...
void record_init_load(string _path)
{
    ...
    for(int i=0; i<count; i++)
    {
        ...
        // 예외 디렉토리 ( 미구현 ) 
        if (strcmp(namelist[i]->d_name, "JsonSchemas") == 0 || strcmp(namelist[i]->d_name, "schemas") == 0 
            || strcmp(namelist[i]->d_name, "odata") == 0 || strcmp(namelist[i]->d_name, "$metadata") == 0)
            continue;
