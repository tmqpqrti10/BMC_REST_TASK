...
LogEntry *LogService::new_log_entry(IpmiLogEvent _ev) {
  ...
  log_entry->message = _ev.msg;
  log_entry->message_args = _ev.message_args;
...
void LogEntry::set_message(string _val) { this->message = _val; }
void LogEntry::set_message_id(string _val) { this->message_id = _val; }
...
void execute_iptables(NetworkProtocol *_net, int _index, string _op) {
  ...
  case SNMP_INDEX:
    if (_net->snmp_enabled == true) {
      cmd_input =
          make_iptable_cmd(_op, "INPUT", SNMP_INDEX, _net->snmp_port, 0);
      cmd_output =
          make_iptable_cmd(_op, "OUTPUT", SNMP_INDEX, _net->snmp_port, 0);
    } else {
      cmd_input =
          make_iptable_cmd(_op, "INPUT", SNMP_INDEX, _net->snmp_port, 1);
      cmd_output =
          make_iptable_cmd(_op, "OUTPUT", SNMP_INDEX, _net->snmp_port, 1);
    }
    break;
  ...
  case NTP_INDEX:
    if (_net->ntp_enabled == true) {
      ...
      cmd_output =
          make_iptable_cmd(_op, "OUTPUT", NTP_INDEX, _net->ntp_port, 0);
    } else {
      cmd_input = make_iptable_cmd(_op, "INPUT", NTP_INDEX, _net->ntp_port, 1);
      cmd_output =
          make_iptable_cmd(_op, "OUTPUT", NTP_INDEX, _net->ntp_port, 1);
    }
    break;
  default:
    break;
