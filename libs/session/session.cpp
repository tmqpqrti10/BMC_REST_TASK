...
uint8_t Ipmisession::getActiveSessCount()
{
    return this->active_session_count;
}
uint8_t Ipmisession::getSessionSlotCount()
{
    return this->session_slot_count;
}

uint8_t Ipmisession::getSessionUserId()
{
    return this->user_id;
}

void Ipmisession::setSessionUserId(uint8_t _id)
{
    this->user_id = _id;
}
...
uint8_t Ipmisession::getSessionPriv()
{
    return this->rmcpData.rolem & 0x07;
}
...
std::vector<uint8_t> Ipmisession::buildHmacRAKP4()
{
    ...
    ret.insert(ret.begin(), this->rmcpData.hmac_rm.begin(), this->rmcpData.hmac_rm.end());
    it = ret.end();
    ret.insert(it, this->rmcpData.mgntSessionId.begin(), this->rmcpData.mgntSessionId.end());
    it = ret.end();
...
std::vector<uint8_t> Ipmisession::buildHmacRAKP3()
{
    ...
    ret.insert(ret.begin(), this->rmcpData.hmac_rc.begin(), this->rmcpData.hmac_rc.end());
    it = ret.end();
    ret.insert(it, this->rmcpData.clntSessionId.begin(), this->rmcpData.clntSessionId.end());
    ret.push_back(this->rmcpData.rolem);
    ret.push_back(this->rmcpData.userNameLength);
    it = ret.end();
    ret.insert(it, this->rmcpData.rmcpUsername.begin(), this->rmcpData.rmcpUsername.end());
...
std::vector<uint8_t> Ipmisession::buildHmacData()
{
    ...
    printf("\n");
#endif
    hmac.insert(it, this->rmcpData.clntSessionId.begin(), this->rmcpData.clntSessionId.end());
    it = hmac.end();
    hmac.insert(it, this->rmcpData.mgntSessionId.begin(), this->rmcpData.mgntSessionId.end());
    it = hmac.end();
    ...
    it = hmac.end();
    hmac.insert(it, this->rmcpData.rmcpUsername.begin(), this->rmcpData.rmcpUsername.end());

    return hmac;
}

uint16_t Ipmisession::getSessionPort()
{
    return this->lan_data.console_port;
}
...
bool Ipmisession::setRmcpLanData(std::vector<uint8_t> _ip, std::vector<uint8_t> _mac, uint16_t _port)
{
...
bool Ipmisession::setRmcpData(std::vector<uint8_t> _mgntId, std::vector<uint8_t> _clntId, 
        std::vector<uint8_t> _rc, std::vector<uint8_t> _rm, uint8_t _rolem, uint8_t _uPriv, std::vector<uint8_t> _userName, uint8_t _nameLength, std::string _passwd)
{
