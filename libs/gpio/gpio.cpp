...
#define AST_GPIO_BANK(index_no, data, dir, int_en, int_type, int_sts, rst_tol, debounce, cmd_s) \
        {                                                                                       \
                .index = index_no,                                                              \
                .data_offset = data,                                                            \
                .dir_offset = dir,                                                              \
...
unsigned int
ast_pwm_read(unsigned int offset)
{
...
static inline void
ast_pwm_write(unsigned int val, unsigned int offset)
{
        unsigned int data;
...
void set_pwm_duty(int fannumb, int value)
{
        ...
        if (val >= 0 && val <= 60)
        {
                ...
                //		printf(" read PWM duty val=%d, regs= 0x%08x\n", value, ast_pwm_read(8));
        }
}

/*static inline*/ unsigned int
ast_gpio_read(unsigned int offset)
{
        unsigned int data;
...
static inline unsigned int
ast_reg_read(unsigned int reg_base, unsigned offset)
{
        unsigned int data;
...
/*static inline*/ void
ast_gpio_write(unsigned int val, unsigned int offset)
{
        unsigned int data;

        //     data = *((unsigned int *) (gpio_reg_base + offset));
        cout << ("gpio_write : base = 0x%08x, offset = 0x%08x val = 0x%08x\n", gpio_reg_base, offset, val) << endl;
        // GPIODBUG("gpio_write: base = 0x%08x, offset = 0x%08x, val = 0x%08x\n", gpio_reg_base, offset, val);
...
int ast_gpio_direction_input(unsigned int gpio_pin)
{
        ...
        v &= ~(GPIO_OUTPUT_MODE << pin); // (ast_gpio->data_offset + (ast_gpio->index * 8)));
...
int ast_gpio_open(void)
{
        ...
        //	printf("<7>\n");
        (map_base = mmap(NULL, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, AST_GPIO_BASE));
        ...
        //	}
        (scu_base = mmap(NULL, 0x256, PROT_READ | PROT_WRITE, MAP_SHARED, fd, AST_SCU_BASE));
        ...
        //              printf("scu regs = 0x%x, data = 0x%08x\n", i, ast_reg_read(scu_reg_base, i));
        //    }
        (pwm_base = mmap(NULL, 0x100, PROT_READ | PROT_WRITE, MAP_SHARED, fd, AST_PWM_BASE));
