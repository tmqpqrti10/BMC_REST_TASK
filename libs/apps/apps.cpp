...
Ipminetwork ipmiNetwork[ETH_COUNT] = {Ipminetwork(1), Ipminetwork(6), Ipminetwork(7), Ipminetwork(8)};
extern Ipmiuser ipmiUser[MAX_USER];
Ipmichannel ipmiChannel[9] = {Ipmichannel(MEDIUM_IPMB, PROTOCOL_IPMB_LAN, 0, CHANNEL_SESSIONLESS), Ipmichannel(MEDIUM_802_3_LAN, PROTOCOL_IPMB_LAN, 1, CHANNEL_MULTI_SESSION),
							  Ipmichannel(MEDIUM_SI, PROTOCOL_KCS, 2, CHANNEL_SESSIONLESS), Ipmichannel(3), Ipmichannel(4), Ipmichannel(5), Ipmichannel(MEDIUM_802_3_LAN, PROTOCOL_IPMB_LAN, 6, CHANNEL_MULTI_SESSION),
							  Ipmichannel(MEDIUM_802_3_LAN, PROTOCOL_IPMB_LAN, 7, CHANNEL_MULTI_SESSION), Ipmichannel(MEDIUM_802_3_LAN, PROTOCOL_IPMB_LAN, 6, CHANNEL_MULTI_SESSION)};
...
void Ipmiapplication::app_get_sysinfo_param(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
	...
	if (rev == 0)
	{
		...
		case SYS_INFO_PARAM_SYSFW_VER:
			*data++ = this->g_sysinfo.sysfw_ver.length();
			std::copy(this->g_sysinfo.sysfw_ver.begin(), this->g_sysinfo.sysfw_ver.end(), data);
			break;

		case SYS_INFO_PARAM_SYS_NAME:
			*data++ = this->g_sysinfo.sys_name.length();
			std::copy(this->g_sysinfo.sys_name.begin(), this->g_sysinfo.sys_name.end(), data);
			break;

		case SYS_INFO_PARAM_PRI_OS_NAME:
			*data++ = this->g_sysinfo.pri_os_name.length();
			std::copy(this->g_sysinfo.pri_os_name.begin(), this->g_sysinfo.pri_os_name.end(), data);
			break;

		case SYS_INFO_PARAM_PRESENT_OS_NAME:
			*data++ = this->g_sysinfo.present_os_name.length();
			std::copy(this->g_sysinfo.present_os_name.begin(), this->g_sysinfo.present_os_name.end(), data);
			break;

		case SYS_INFO_PARAM_PRESENT_OS_VER:
			*data++ = this->g_sysinfo.present_os_ver.length();
			std::copy(this->g_sysinfo.present_os_ver.begin(), this->g_sysinfo.present_os_ver.end(), data);
			break;
		}
	}
}

void Ipmiapplication::app_set_sysinfo_param(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
	...
	case SYS_INFO_PARAM_PRI_OS_NAME:
		...
		break;
	case SYS_INFO_PARAM_PRESENT_OS_NAME:
		this->g_sysinfo.present_os_name.assign(req->data + 4, req->data + 4 + str_len);
		break;
	case SYS_INFO_PARAM_PRESENT_OS_VER:
		this->g_sysinfo.present_os_ver.assign(req->data + 4, req->data + 4 + str_len);
		break;
...
void Ipmiapplication::app_set_useraccess(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_set_user_passwd(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_set_username(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_get_username(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_get_useraccess(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_get_session_info(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_clear_message_flag(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
	cout << "app_clear_message_flag" << endl;
	ipmi_req_t *req = (ipmi_req_t *)request;
	ipmi_res_t *res = (ipmi_res_t *)response;
	uint8_t param = req->data[0];
	uint8_t oem[3] = {((param & 0x20) >> 5), ((param & 0x40) >> 6), ((param & 0x80) >> 7)};
...
void Ipmiapplication::app_set_global_enables(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_get_global_enables(uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_close_session(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_set_session_priv(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_set_channel_access(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_get_channel_access(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
 ...
 * 
 * @author KICHEOL
 * @bug event 를 위한 channel은 14인데 현재는 0~8까지의 channel만보유 따라서 임시로 ipmichannel[0]으로 수정 오류 발생예정
 */
void Ipmiapplication::app_get_channel_info(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::app_get_channel_auth_cap(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Ipmiapplication::ipmi_handle_app(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
static void ipmi_handle_sensor(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
...
static void ipmi_handle_storage(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
...
static void ipmi_handle_transport(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
...
void ipmi_handle_rest(rest_req_t *req, uint8_t *response, int *res_len)
{
	...
	case CMD_TRY_LOGIN:
	{
		...
		*res_len = strlen(response);
		// for (int i = 0; i < 10; i++)
		// {
		// 	cout << "USERNAME =" << ipmiUser[i].getUsername() << "PASSWORD=" << ipmiUser[i].getUserpassword() << endl;
		// }
	...
	case CMD_SET_FRU_BOARD:
	{
		...
		char mfg[LEN_MFG], product[LEN_PRODUCT], serial[LEN_SERIAL], part_num[LEN_PART_NUM];
		...
		memcpy(serial, req->data + 1 + LEN_MFG_DATE + LEN_MFG + LEN_PRODUCT, LEN_SERIAL);
		memcpy(part_num, req->data + 1 + LEN_MFG_DATE + LEN_MFG + LEN_PRODUCT + LEN_SERIAL, LEN_PART_NUM);
		c_year = atoi(year);
		c_month = atoi(month);
		c_day = atoi(day);
		c_hour = atoi(hour);
		...
		sprintf(datetime, "%04d-%02d-%02d %02d:%02d:%02d", c_year, c_month, c_day, c_hour, c_min, c_sec);
	...
	case CMD_SET_FRU_PRODUCT:
	{
		...
		int fru_id = (int)req->data[0];
		char name[LEN_NAME], mfg[LEN_MFG], version[LEN_VERSION], serial[LEN_SERIAL], part_num[LEN_PART_NUM];
		...
		memcpy(serial, req->data + 1 + LEN_NAME + LEN_MFG + LEN_VERSION, LEN_SERIAL);
		memcpy(part_num, req->data + 1 + LEN_NAME + LEN_MFG + LEN_VERSION + LEN_SERIAL, LEN_PART_NUM);
	...
	case CMD_SET_POWER_STATUS:
	{
		...
		ipmiChassis.chassis_control((ipmi_req_t *)req, (ipmi_res_t *)response, (uint8_t *)res_len);
		cout << "\t response=" << (char *)response << endl;
		cout << "\t res_len=" << (*res_len) << endl;
	...
	case CMD_SENSOR_SET_THRESH:
	{
		...
		{
			printf("th_data %d : %f\n", i, th_data[i]);
			// printf("convert_data %d : %f\n", i, sdr_convert_sensor_value_to_raw(p_sdr, th_data[i]));
		}
	...
	case CMD_SET_LAN_MAC_ADDR:
	{
		...
		printf("SET MAC Address - Ethernet : %x / MAC: %x:%x:%x:%x:%x:%x\n", req->data[0], req->data[1], req->data[2], req->data[3], req->data[4], req->data[5], req->data[6]);
	...
	case CMD_SET_LAN_IPV4_IP_NETMASK:
	{
		...
		printf("REST Set IP Address : %x / IP: %d.%d.%d.%d\n", req->data[0], IP_ADDRS[0], IP_ADDRS[1], IP_ADDRS[2], IP_ADDRS[3]);
		printf("REST Set Netmask Address : %x / IP: %d.%d.%d.%d\n", req->data[0], NET_ADDRS[0], NET_ADDRS[1], NET_ADDRS[2], NET_ADDRS[3]);
	...
	case CMD_SET_LAN_VLAN_ID:
	{
		...
		sprintf(cmds, "ifconfig eth0.%d %d.%d.%d.%d netmask %d.%d.%d.%d up", ipmiNetwork[req->data[1]].vlan_id, ipmiNetwork[req->data[1]].ip_addr[0], ipmiNetwork[req->data[1]].ip_addr[1], ipmiNetwork[req->data[1]].ip_addr[2], ipmiNetwork[req->data[1]].ip_addr[3], ipmiNetwork[req->data[1]].net_mask[0], ipmiNetwork[req->data[1]].net_mask[1], ipmiNetwork[req->data[1]].net_mask[2], ipmiNetwork[req->data[1]].net_mask[3]);
		system(cmds);
		break;
	}
...
static void ipmi_handle_chassis(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
	uint8_t cmd = request->cmd;
	switch (cmd)
	{
...
uint8_t ipmi_handle(uint8_t bRMCP, uint8_t *request, uint8_t req_len, uint8_t *response, uint8_t *res_len)
{
...
void plat_ipmi_init(void)
{
	...
	t_lanMonitor[0] = std::thread([&]()
								  { ipmiNetwork[0].lanMonitorHandler(); });
	t_lanMonitor[1] = std::thread([&]()
								  { ipmiNetwork[1].lanMonitorHandler(); });
	t_lanMonitor[2] = std::thread([&]()
								  { ipmiNetwork[2].lanMonitorHandler(); });
	t_lanMonitor[3] = std::thread([&]()
								  { ipmiNetwork[3].lanMonitorHandler(); });
	...
	{
		inner.insert(std::make_pair(rec_id, Ipmisdr(rec_id, peb_sensors[i].sensor_num, peb_sensors[i])));
		sdr_rec.insert(std::make_pair(rec_id, inner));
		rec_id++;
	}
	for (int i = 0; i < PDPB_SENSOR_COUNT; i++)
	{
		inner.insert(std::make_pair(rec_id, Ipmisdr(rec_id, pdpb_sensors[i].sensor_num, pdpb_sensors[i])));
		sdr_rec.insert(std::make_pair(rec_id, inner));
		rec_id++;
	}
	for (int i = 0; i < NVA_SENSOR_COUNT; i++)
	{
		inner.insert(std::make_pair(rec_id, Ipmisdr(rec_id, nva_sensors[i].sensor_num, nva_sensors[i])));
		sdr_rec.insert(std::make_pair(rec_id, inner));
		rec_id++;
	}
	// plat_fru_device_init();
	// cout<<"fru devcie_init !!"<<endl;
	// //TEST용 fru 후에 삭제
	// for (int i = 0; i < SENSOR_FRU_MAX; i++)
	// {
	// 	inner.insert(std::make_pair(rec_id, Ipmisdr(rec_id, fru_sensors[0].sensor_num, fru_sensors[0])));
	// 	sdr_rec.insert(std::make_pair(rec_id, inner));
	// 	rec_id++;
	// }
}
...
int authenticate_radius(std::string _username, std::string _password)
{
	puts("auth rad");
	char cmd[64];
	sprintf(cmd, "echo %s | auth %s radius", _password.c_str(), _username.c_str());
	if (system(cmd) == 0)
	{
		return 2;
	}
...
void get_temp_cpu0(json::value &JCPU)
{
	...
	if (status == 1)
	{
		...
		JCPU["VALUE"] = json::value::string(to_string(f_val));
	}
...
void get_fans(json::value &JFANS)
{
	...
	fan[0] = sdr_sensor_read(NVA_SENSOR_BP_FAN1) * 100;
	fan[1] = sdr_sensor_read(NVA_SENSOR_BP_FAN2) * 100;
 ...
 * @bug  net_priority= 8로 고정되어있는 문제 특별한 문제 없으면 사용가능 kernel name 에러
 *  
 */
void get_sys_info(json::value &JSYS_INFO)
{
	...
	sprintf((char *)temp.c_str(), "%02x.%02x.%02x.%02x.%02x.%02x\0", iptemp.at(0), iptemp.at(1), iptemp.at(3), iptemp.at(4), iptemp.at(5));
	strcat(temp.c_str(), "\0");
	temp.shrink_to_fit();
	...
	sprintf((char *)temp.c_str(), "%d.%d.%d.%d\0", iptemp.at(0), iptemp.at(1), iptemp.at(2), iptemp.at(3));
	strcat(temp.c_str(), "\0");
	temp.shrink_to_fit();
	...
	subobj["IPMIFW_BLDTIME"] = json::value::string(U(std::string(g_firmware_build_time)));
	subobj["KERNAL_VERSION"] = json::value::string(U(std::string(ipmiApplication.g_kernel_version)));
	JSYS_INFO["MC"] = subobj;
...
void get_health_summary(json::value &jhealth_summary)
{
	...
	for (snr_id = PDPB_SENSOR_TEMP_CPU0_CH0_DIMM0; snr_id <= PDPB_SENSOR_TEMP_CPU0_CH3_DIMM2; snr_id++)
	{
	...
	for (snr_id = PDPB_SENSOR_TEMP_CPU1_CH0_DIMM0; snr_id <= PDPB_SENSOR_TEMP_CPU1_CH3_DIMM2; snr_id++)
	{
		sensor_index = plat_find_sdr_index(snr_id);
		temp = sdr_rec[sensor_index].find(sensor_index)->second.get_sensor_thresh_t();
		if (temp.nominal > 0)
	...
	for (snr_id = PDPB_SENSOR_TEMP_CPU0; snr_id <= PDPB_SENSOR_TEMP_CPU1; snr_id++)
	{
		sensor_index = plat_find_sdr_index(snr_id);
		temp = sdr_rec[sensor_index].find(sensor_index)->second.get_sensor_thresh_t();
		if (temp.nominal > 0)
	...
	for (snr_id = NVA_SENSOR_PSU1_WATT; snr_id <= NVA_SENSOR_PSU2_WATT; snr_id++)
	{
		sensor_index = plat_find_sdr_index(snr_id);
		temp = sdr_rec[sensor_index].find(sensor_index)->second.get_sensor_thresh_t();
		if (temp.nominal > 0)
			JPOWER.push_back(json::value::number(1));
		else
...
int find_sensor_name(uint8_t sType, uint8_t sNum, string &msg)
{
	...
	// Sensor Type 9= P12PSU
		// cout << "warning: find_sensor_name sensor s_Type" <<static_cast<int>(sType)<< endl;
		// sensor_index = plat_find_sdr_index(sNum);
		// temp = sdr_rec[sensor_index].find(sensor_index)->second.get_sensor_thresh_t();
		// if (sensor_index < 0)
		// {
		// 	cout << "sensor s_Type: can't find sensor. sensor name =" <<temp.str<< endl;
		// 	return 0;
		// }
 ...
 */
int new_ipmi_user(string user_name, string password, bool enabled)
{
	int count = ++usercount;
	if (count > 10)
		return 0;
...
int rest_get_sysinfo_config(char *res)
{
	...
	power_info["STATUS"] = json::value::string(U(ipmiChassis.get_power_status() ? "on" : "off"));
	...
	if (net_priority == 1)
	{
		...
		sprintf(mac_str, "%u.%u.%u.%u.%u.%u", ipmiNetwork[0].ip_addr[0], ipmiNetwork[0].ip_addr[1], ipmiNetwork[0].ip_addr[2], ipmiNetwork[0].ip_addr[3], ipmiNetwork[0].ip_addr[4], ipmiNetwork[0].ip_addr[5]);
	...
	{
		sprintf(ip_str, "%u.%u.%u.%u", ipmiNetwork[1].ip_addr[0], ipmiNetwork[1].ip_addr[1], ipmiNetwork[1].ip_addr[2], ipmiNetwork[1].ip_addr[3]);
		sprintf(mac_str, "%u.%u.%u.%u.%u.%u", ipmiNetwork[1].ip_addr[0], ipmiNetwork[1].ip_addr[1], ipmiNetwork[1].ip_addr[2], ipmiNetwork[1].ip_addr[3], ipmiNetwork[1].ip_addr[4], ipmiNetwork[1].ip_addr[5]);
	...
	strncpy(res, obj.serialize().c_str(), obj.serialize().length());
	// #else
	// 	strcpy(res, "{\"POWER_INFO\": {\n\t\"STATUS\": \"Not supported\"\n\t},\n\t\"GENERIC_INFO\":{\n\t\t\"GENERIC\": {\n\t\t\t\"IPMIFW_TAG\":\"Not Supported\",\n\t\t\t\"BMC_IP\":\"Not Supported\",\n\t\t\t\"BMC_MAC\":\"Not Supported\",\n\t\t\t\"FRU_VERSION\":\"Not Supported\",\n\t\t\t\"IPMIFW_VERSION\":\"Not Supported\",\n\t\t\t\"BIOS_VERSION\":\"Not Supported\",\n\t\t\t\"SDR_VERSION\":\"Not Supported\",\n\t\t\t\"WEB_VERSION\":\"Not Supported\",\n\t\t\t\"IPMIFW_BLDTIME\":\"Not Supported\"\n\t\t},\n\t\"KERNEL\": {\n\t\t\"VERSION\":\"Not Supported\"\n\t\t}\n\t}\n}");
	// #endif
	return strlen(res);
}
...
int rest_get_ddns_info(unsigned char *res)
{
	...
	string s_ipaddr_v6(ipmiNetwork[0].ip_addr_v6.begin(), ipmiNetwork[0].ip_addr_v6.end());
...
int rest_get_lan_config(char *res)
{
	...
	for (int i = 0; i < MAX_NIC; i++)
	{
		...
		if (get_ipv6_info(channel, (unsigned char *)r_ip_addr6, (unsigned char *)r_netmask6, (unsigned char *)r_gateway6) == -1)
		{
			fprintf(stderr, "get ipv6 info failed\n");
			return 0;
		}

		// printf("ip : %s\nnetmask : %s\ngateway : %s\n", r_ip_addr6, r_netmask6, r_gateway6);
		// printf("\t\t\t dy : get lan checkpoint 1 in channel %d===========\n", channel);

		ipmiNetwork[i].ip_addr_v6.assign(r_ip_addr6, r_ip_addr6 + SIZE_IP_ADDR_V6);
		ipmiNetwork[i].net_mask_v6.assign(r_netmask6, r_netmask6 + SIZE_NET_MASK_V6);
		ipmiNetwork[i].df_gw_ip_addr_v6.assign(r_gateway6, r_gateway6 + SIZE_IP_ADDR_V6);

		// printf("\t\t\t dy : get lan checkpoint 2 ===========\n");
		// string ip6(ipmiNetwork[i].ip_addr_v6.begin(), ipmiNetwork[i].ip_addr_v6.end());
		// string nm6(ipmiNetwork[i].net_mask_v6.begin(), ipmiNetwork[i].net_mask_v6.end());
		// string gw6(ipmiNetwork[i].df_gw_ip_addr_v6.begin(), ipmiNetwork[i].df_gw_ip_addr_v6.end());
	...
	for (int i = 0; i < MAX_NIC; i++)
	{
		...
		GENERIC["LAN_SETTING_ENABLE"] = json::value::string(U(to_string(ipmiNetwork[i].set_enable)));
		sprintf(buf, "%02x:%02x:%02x:%02x:%02x:%02x", ipmiNetwork[i].mac_addr[0], ipmiNetwork[i].mac_addr[1], ipmiNetwork[i].mac_addr[2], ipmiNetwork[i].mac_addr[3], ipmiNetwork[i].mac_addr[4], ipmiNetwork[i].mac_addr[5]);
		GENERIC["MAC_ADDRESS"] = json::value::string(U(buf));

		memset(buf, 0, sizeof(buf));

		IPV4["IPV4_PREFERRED"] = json::value::string(U(""));
		sprintf(buf, "%d.%d.%d.%d", ipmiNetwork[i].df_gw_ip_addr[0], ipmiNetwork[i].df_gw_ip_addr[1], ipmiNetwork[i].df_gw_ip_addr[2], ipmiNetwork[i].df_gw_ip_addr[3]);
		IPV4["IPV4_GATEWAY"] = json::value::string(U(buf));
		sprintf(buf, "%d.%d.%d.%d", ipmiNetwork[i].net_mask[0], ipmiNetwork[i].net_mask[1], ipmiNetwork[i].net_mask[2], ipmiNetwork[i].net_mask[3]);
		IPV4["IPV4_NETMASK"] = json::value::string(U(buf));
		sprintf(buf, "%d.%d.%d.%d", ipmiNetwork[i].ip_addr[0], ipmiNetwork[i].ip_addr[1], ipmiNetwork[i].ip_addr[2], ipmiNetwork[i].ip_addr[3]);
		IPV4["IPV4_ADDRESS"] = json::value::string(U(buf));
		IPV4["IPV4_DHCP_ENABLE"] = json::value::string(U(to_string(ipv4_srcs)));

		string s_subnet_mask_v6(ipmiNetwork[i].net_mask_v6.begin(), ipmiNetwork[i].net_mask_v6.end());
		IPV6["IPV6_SUBNET_PREFIX_LENGTH"] = json::value::string(s_subnet_mask_v6);
		string s_ip_addr_v6(ipmiNetwork[i].ip_addr_v6.begin(), ipmiNetwork[i].ip_addr_v6.end());
		IPV6["IPV6_ADDRESS"] = json::value::string(s_ip_addr_v6);
		IPV6["IPV6_ENABLE"] = json::value::string(U(to_string(ipmiNetwork[i].set_enable_v6)));
		IPV6["IPV6_DHCP_ENABLE"] = json::value::string(U(to_string(ipmiNetwork[i].ip_src_v6)));
		string s_gateway(ipmiNetwork[i].df_gw_ip_addr_v6.begin(), ipmiNetwork[i].df_gw_ip_addr_v6.end());
		IPV6["IPV6_GATEWAY"] = json::value::string(s_gateway);

		VLAN["VLAN_SETTINGS_ENABLE"] = json::value::string(U(to_string(vlan_enables)));
		VLAN["VLAN_ID"] = json::value::string(U(to_string(ipmiNetwork[i].vlan_id)));
		VLAN["VLAN_PRIORITY"] = json::value::string(U(to_string(ipmiNetwork[i].vlan_priority)));
...
const char *etable[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec", NULL};
...
{
	char machine_name[50] = "\0", sender_address[50] = "\0", primary_server_address[50] = "\0", primary_user_name[50] = "\0";
	char primary_user_password[50] = "\0", secondary_server_address[50] = "\0", secondary_user_name[50] = "\0", secondary_user_password[50] = "\0";
	...
	if (flag == 0)
	{
		...
		PRIMARY["PRIMARY_SERVER_ADDRESS"] = json::value::string(U(primary_server_address));
		PRIMARY["PRIMARY_USER_NAME"] = json::value::string(U(primary_user_name));
		PRIMARY["PRIMARY_USER_PASSWORD"] = json::value::string(U(primary_user_password));

		json ::value SECONDARY = json::value::object();
		SECONDARY["SECONDARY_SERVER_ADDRESS"] = json::value::string(U(secondary_server_address));
		SECONDARY["SECONDARY_USER_NAME"] = json::value::string(U(secondary_user_name));
		SECONDARY["SECONDARY_USER_PASSWORD"] = json::value::string(U(secondary_user_password));
...
{
	char country[3] = "\0", state_province[32] = "\0", city_locality[32] = "\0", organ[32] = "\0";
	char organ_unit[32] = "\0", common[32] = "\0", email[32] = "\0", keylen[5] = "\0";
	char valid_from[16] = "\0", valid_to[16] = "\0";
	int valid_for = 0;
	FILE *fp_read = fopen(SSL_BIN, "r");
	CertificateService* certificate=((CertificateService *)g_record[ODATA_CERTIFICATE_SERVICE_ID]);
...
int set_ssl_1(char *keylen, char *country, char *state_province, char *city_locality, char *organ, int valid_for)
{
...
void get_expire_day(int days, char *today, char *expire_day)
{
	...
	now = time(NULL);
	now_t = localtime(&now);
	sprintf(today, "%d-%d-%d", now_t->tm_year + 1900, now_t->tm_mon + 1, now_t->tm_mday);
...
int set_ldap_port(char *port)
{
	...
	else
	{
		if (fread(&ldap_config, sizeof(ldap_config_t), 1, fp_read) < 1)
		{
			fprintf(stderr, "\t\tError : fread ldap_config for setting port failed\n");
		}
		fclose(fp_read);
	}
...
int set_ldap_searchbase(char *basedn)
{
	...
	else
	{
		if (fread(&ldap_config, sizeof(ldap_config_t), 1, fp_read) < 1)
		{
			fprintf(stderr, "\t\tError : fread ldap_config for setting searchbase failed\n");
		}
		fclose(fp_read);
	}
...
int set_ldap_binddn(char *binddn)
{
	...
	else
	{
		if (fread(&ldap_config, sizeof(ldap_config_t), 1, fp_read) < 1)
		{
			fprintf(stderr, "\t\tError : fread ldap_config for setting binddn failed\n");
		}
		fclose(fp_read);
	}
...
int set_ldap_bindpw(char *bindpw)
{
	...
	else
	{
		if (fread(&ldap_config, sizeof(ldap_config_t), 1, fp_read) < 1)
		{
			fprintf(stderr, "\t\tError : fread ldap_config for setting bindpw failed\n");
		}
		fclose(fp_read);
	}
...
int set_ldap_timelimit(int time)
{
	...
	else
	{
		if (fread(&ldap_config, sizeof(ldap_config_t), 1, fp_read) < 1)
		{
			fprintf(stderr, "\t\tError : fread ldap_config for setting time failed\n");
		}
		fclose(fp_read);
	}
...
// 
static void
app_get_watchdog_timer_params(unsigned char *request, unsigned char *response,
...
#include <bitset>
static void
app_set_watchdog_timer_params(unsigned char *request, unsigned char *response,
							  unsigned char *res_len)
{
	cout << ("Get watchdog timer parameter") << endl;
	ipmi_req_t *req = (ipmi_req_t *)request;
...
void plat_lan_alert_init(){
	...
	for(i = 0 ; i < ETH_COUNT ; i++){
		memcpy(ipmiNetwork[i].dest_type, g_lan_alert[i].dest_type, sizeof(g_lan_alert[i].dest_type));
		memcpy(ipmiNetwork[i].dest_addr, g_lan_alert[i].dest_addr, sizeof(g_lan_alert[i].dest_addr));
	}
	fclose(l_a);
}
/////////////////////////////////////////////////////////////PSU관련///////////
...
int rm_count_1, rm_count_2, rh_count_1, rh_count_2, rd_count_1, rd_count_2, upper_int = 0;
            ...
            case WATT_ALL:
                ...
                while(1){
                    ...
                    if(SQLITE_ROW == rc){
                        ...
                        strcpy(day_power_1[loop]->dt, (char *)sqlite3_column_text(statement, 2));
                        loop++;
                    }
                    else if(SQLITE_BUSY == rc || SQLITE_LOCKED == rc)
                        continue;
                ...
                while(1){
                    ...
                    if(SQLITE_ROW == rc){
                        ...
                        strcpy(day_power_2[loop]->dt, (char *)sqlite3_column_text(statement, 2));
                        loop++;
                    }
                    else if(SQLITE_BUSY == rc || SQLITE_LOCKED == rc)
                        continue;
                ...
                while(1){
                    ...
                    if(SQLITE_ROW == rc){
                        ...
                        strcpy(hour_power_1[loop]->dt, (char *)sqlite3_column_text(statement, 2));
                ...
                while(1){
                    ...
                    if(SQLITE_ROW == rc){
                        ...
                        strcpy(hour_power_2[loop]->dt, (char *)sqlite3_column_text(statement, 2));
                ...
                loop = 0;
                 while(1){
                    ...
                    if(SQLITE_ROW == rc){
                        ...
                        strcpy(g_min_power_1[loop].dt, (char *)sqlite3_column_text(statement, 2));
                ...
                while(1){
                    ...
                    if(SQLITE_ROW == rc){
                        ...
                        strcpy(g_min_power_2[loop].dt, (char*)sqlite3_column_text(statement, 2));
...
int get_lowest_power_1(int flag){
    ...
    for(i = 0 ; i < MAX_LAST_MIN_COUNT; i++){
        if(i == 0)
            ...
            if((min_lowest > g_min_power_1[i].watt) && (g_min_power_1[i].watt != 0)){
                min_lowest = g_min_power_1[i].watt;
                low_index_1[1] = i;
            }
        }
    ...
    for(i = 0 ; i < MAX_LAST_HOUR_COUNT; i++){
        if(i == 0)
            ...
            if((hour_lowest > g_hour_power_1[i].watt) && (g_hour_power_1[i].watt != 0)){
                hour_lowest = g_hour_power_1[i].watt;
                low_index_1[2] = i;
            }
        }
    ...
    for(i = 0 ; i < MAX_LAST_DAY_COUNT; i++){
        if(i == 0)
            ...
            if((day_lowest > g_day_power_1[i].watt) && (g_day_power_1[i].watt != 0)){
                day_lowest = g_day_power_1[i].watt;
                low_index_1[3] = i;
            }
        }
...
int get_lowest_power_2(int flag){
    ...
    for(i = 0 ; i < MAX_LAST_MIN_COUNT; i++){
        if(i == 0)
            ...
            if((min_lowest > g_min_power_2[i].watt) && (g_min_power_2[i].watt != 0)){
                min_lowest = g_min_power_2[i].watt;
                low_index_2[1] = i;
            }
        }
    ...
    for(i = 0 ; i < MAX_LAST_HOUR_COUNT; i++){
        if(i == 0)
            ...
            if((hour_lowest > g_hour_power_2[i].watt) && (g_hour_power_2[i].watt != 0)){
                hour_lowest = g_hour_power_2[i].watt;
                low_index_2[2] = i;
            }
        }
    ...
    for(i = 0 ; i < MAX_LAST_DAY_COUNT; i++){
        if(i == 0)
            ...
            if((day_lowest > g_day_power_2[i].watt) && (g_day_power_2[i].watt != 0)){
                day_lowest = g_day_power_2[i].watt;
                low_index_2[3] = i;
            }
        }
