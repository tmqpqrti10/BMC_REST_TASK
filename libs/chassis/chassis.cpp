...
uint8_t Ipmichassis::get_power_status(void)
{
    ...
    //printf("\t\t\tdy : is redfish_power control ok ? : %d\n", redfish_power_ctrl);
    // if( redfish_power_ctrl ==0)
    //     return 0;
    //adc0.value 값
    if(ps >= 150)
    {
...
void Ipmichassis::chassis_restart_cause(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
...
void Ipmichassis::chassis_get_status(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    uint8_t *data = &response->data[0];
...
void Ipmichassis::chassis_control(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    // uint8_t *data = &response->data[0];
...
void Ipmichassis::chassis_set_policy(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
...
void Ipmichassis::chassis_identify(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
...
void Ipmichassis::chassis_get_poh(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    uint8_t *data = &response->data[0];
...
void Ipmichassis::chassis_get_boot_options(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    uint8_t param = request->data[0];
    uint8_t *data = &response->data[0];
    ...
    *data++ = request->data[0];
    ...
    switch(param)
    {
        ...
        case PARAM_BOOT_INFO_ACK:
            std::copy(this->g_chassis_bootp.boot_info_ack.begin(), this->g_chassis_bootp.boot_info_ack.end(), data);
            data += BOOT_INFO_ACK_SIZE; // ACK Size
            break;
        case PARAM_BOOT_FLAGS:
            std::copy(this->g_chassis_bootp.boot_flags.begin(), this->g_chassis_bootp.boot_flags.end(), data);
            data += BOOT_FLAGS_SIZE; // Boot Flag Size
            break;
        case PARAM_BOOT_INIT_INFO:
            std::copy(this->g_chassis_bootp.boot_init_info.begin(), this->g_chassis_bootp.boot_flags.end(), data);
...
void Ipmichassis::chassis_set_boot_options(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    uint8_t param = request->data[0];
    uint8_t *data = &response->data[0];
    ...
    switch(param)
    {
        ...
        case PARAM_BOOT_FLAG_CLR:
            ...
            break;
        case PARAM_BOOT_INFO_ACK:
            this->g_chassis_bootp.boot_info_ack.assign(request->data + 1, request->data + 1 + BOOT_INFO_ACK_SIZE);
            break;
        case PARAM_BOOT_FLAGS:
            this->g_chassis_bootp.boot_flags.assign(request->data + 1, request->data + 1 + BOOT_FLAGS_SIZE);
            break;
        case PARAM_BOOT_INIT_INFO:
            this->g_chassis_bootp.boot_init_info.assign(request->data + 1, request->data + 1 + BOOT_INIT_INFO_SIZE);
            break;
        default:
            response->cc = CC_PARAM_OUT_OF_RANGE;
            break;
