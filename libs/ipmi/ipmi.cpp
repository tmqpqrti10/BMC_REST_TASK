...
static int LOGcreateFile(struct tm *tml, const char *src_file){
  ...
  snprintf(filename, 1024, "%s/%s-%04d%02d%02d.log", log_folder, log_file_prefix, 1900+tml->tm_year, tml->tm_mon + 1, tml->tm_mday);
...
int LOGlogging(char log_type, const char *src_file, const char *func, int line_no, const char *fmt, ...){
  ...
  sz += fprintf(fp_log_file, "[%04d-%02d-%02d %02d:%02d:%02d]", 1900+tml->tm_year, tml->tm_mon+1, \
  tml->tm_mday, tml->tm_hour, tml->tm_min, tml->tm_sec);
 ...
 * Description : 다음 index를 response의 data에 저장함. 마지막 index는 0xff로 마지막임을 알려줌
 */
 ...
 * Description : 현재 저장된 sel의 갯수. roll over 되지 않게 해놨으므로 항상 end - begin
 */
...
void plat_globalenable_save(void)
{
  ...
  if (record_is_exist(_uri))
  {
    ...
    manager->global_enable=(int)global_enable;
