...
Dcmiconfiguration::Dcmiconfiguration()
{
    this->dcmi_guid.insert(dcmi_guid.begin(), std::begin(uuid_hex), std::end(uuid_hex));
    this->dcmi_mngctrl.insert(dcmi_mngctrl.begin(), std::begin(uuid_str), std::end(uuid_str));
...
DCMI_CAPA_PATH 파일이 존재할 경우 dcmi_capabilities_t 구조체에 파일 내용을 복사한다.\n
* @param void
* @return void
*/
...
/**
* @brief DCMI Discover 및 Compact 명령어 전송 시 DCMI 전체 설정 정보를 반환하는 함수.\n
IPMI Request 데이터에 의해 전송된 Selector에 따라 Capabilites 정보를 반환한다.\n
...
* @return void
*/
void Dcmiconfiguration::dcmi_discover(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
    ...
    cout<<"grp_extension="<<(int)grp_extension<<endl;
...
void Dcmiconfiguration::dcmi_set_power_limit(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
    ipmi_req_t *req = (ipmi_req_t *) request;
    ipmi_res_t *res = (ipmi_res_t *) response;

    uint16_t limit_val = req->data[6] | (req->data[7] << 8);
    uint32_t correction_val = (req->data[11] << 24) | (req->data[10] << 16) | (req->data[9] << 8) | req->data[8];
    uint16_t sample_time = (req->data[15] << 8) | req->data[14];
    char buf[500] = {
        0,
    };
    sprintf(buf, "DCMI Set Power Limit : %d ", limit_val);
    //ipmiLogEventHandler.Event_Registration(IpmiLogEvent(buf, "Power", "System"));
    if (limit_val > 0xffff)
        res->cc = CC_POWER_OOL;
...
* @return void
*/
void Dcmiconfiguration::plat_dcmi_data_save()
{
    cout<<"미구현"<<endl;
}
void Dcmiconfiguration::dcmi_get_power_reading(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
    ipmi_req_t *req = (ipmi_req_t *) request;
    ipmi_res_t *res = (ipmi_res_t *) response;
    uint8_t *data = &res->data[0];
...
void Dcmiconfiguration::dcmi_power_get_limit(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
...
void Dcmiconfiguration::dcmi_get_mngctrl(uint8_t *request, uint8_t *response, uint8_t *res_len)
{
    ipmi_req_t *req = (ipmi_req_t *) request;
    ipmi_res_t *res = (ipmi_res_t *) response;
    ...
    else
    {
        ...
        std::copy(dcmi_mngctrl.begin() + req->data[1], dcmi_mngctrl.begin() + req->data[1] + req->data[2], res->data);
        *res_len = 2 + req->data[2];
    }
}
	void
Dcmiconfiguration::dcmi_get_disvry (unsigned char *request, unsigned char *response,
...
void
Dcmiconfiguration::dcmi_power_activate (unsigned char *request, unsigned char *response,
		unsigned char *res_len)
{
	ipmi_req_t *req = (ipmi_req_t *) request;
	ipmi_res_t *res= (ipmi_res_t *) response;
