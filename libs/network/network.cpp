...
void Ipminetwork::plat_network_save(void)
{
    ...
    if (this->chan_index == 0)
        ...
        _uri = ODATA_ETHERNET_INTERFACE_ID + string("/NIC") + to_string(this->chan_index);
    ...
    if (record_is_exist(_uri))
    {
        ...
        sprintf(buf, "%d.%d.%d.%d", df_gw_ip_addr[0], df_gw_ip_addr[1], df_gw_ip_addr[2], df_gw_ip_addr[3]);
        if( ether->v_ipv4.size()>1)
        ether->v_ipv4[0].gateway=buf;
        }
        memset(buf, 0, sizeof(buf));
        if(net_mask.size()>2){
        sprintf(buf, "%d.%d.%d.%d",net_mask[0], net_mask[1], net_mask[2], net_mask[3]);
        if( ether->v_ipv4.size()>1)
...
void get_gateway(uint8_t chan, uint8_t * gateway)
{
	...
	if(fgets(line, sizeof(line), fp) != NULL){
		for(i=0; i<1000; i++){
			if((line[i] !='.') && (line[i]<'0' || line[i]>'9')) break;
		}
		line[i]=0;
...
int set_eth_priority(unsigned char dev_num) {
    ...
    if(dev_num == 1)
	system("sed -i \"16s/.*/	\\/sbin\\/ifup eth0/g\" /etc/init.d/S40network");
    else if(dev_num == 8)
	system("sed -i \"16s/.*/	\\/sbin\\/ifup eth1/g\" /etc/init.d/S40network");
    return 0;
}
...
void Ipminetwork::get_lan_config(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    ...
    *data++ = 0x01;
    ...
    switch(param)
    {
        ...
        case LAN_PARAM_BACK_GW_MAC_ADDR:
            std::copy(this->back_gw_mac_addr.begin(), this->back_gw_mac_addr.end(), data);
            data += SIZE_MAC_ADDR;
            break;
        ...
        case LAN_PARAM_DEST_TYPE:
            std::copy(this->dest_type[select].begin(), this->dest_type[select].end(), data);
            // plat_lan_alert_save();
            data += SIZE_DEST_TYPE;
            break;
        case LAN_PARAM_DEST_ADDR:
            std::copy(this->dest_addr[select].begin(), this->dest_addr[select].end(), data);
            // plat_lan_alert_save();
            data += SIZE_DEST_ADDR;
            break;
...
void Ipminetwork::set_lan_config(ipmi_req_t *request, ipmi_res_t *response, uint8_t *res_len)
{
    uint8_t param = request->data[1];
    ...
    switch(param)
    {
        ...
        case LAN_PARAM_IP_ADDR:
            if((request->data[2] == 0) && (request->data[3] == 0) && (request->data[4] == 0) && (request->data[5] == 0))
                response->cc = CC_PARAM_OUT_OF_RANGE;
            else if((request->data[2] == 0xff) && (request->data[3] == 0xff) && (request->data[4] == 0xff) && (request->data[5] == 0xff))
                response->cc = CC_PARAM_OUT_OF_RANGE;
            else
            {
        ...
        case LAN_PARAM_NET_MASK:
            if((request->data[2] == 0xff) && (request->data[3] == 0xff) && (request->data[4] == 0xff) && (request->data[5] == 0xff))
            {
                response->cc = CC_INVALID_PARAM;
                break;
            }
        ...
        case LAN_PARAM_DF_GW_IP_ADDR:
            if(((request->data[2] == 0) && (request->data[3] == 0) && (request->data[4] == 0) && (request->data[5] == 0)) ||
            ((request->data[2] == 0xff) && (request->data[3] == 0xff) && (request->data[4] == 0xff) && (request->data[5] == 0xff)))
            {
                response->cc = CC_INVALID_PARAM;
                break;
            }
...
void Ipminetwork::lanMonitorHandler()
{
    ...
    while(1)
    {
        ...
        char dev[5] = {0,};
        char command[128] = {0,};
        char str_ip[20] = {0,}, str_net[20] = {0,}, str_gw[20] = {0,};
        uint8_t *b_ip = new uint8_t[4], *b_netmask = new uint8_t[4], *b_gw = new uint8_t[4], *b_mac = new uint8_t[6];
        uint8_t *t_ip = new uint8_t[4], *t_netmask = new uint8_t[4], *t_gw = new uint8_t[4], *t_mac = new uint8_t[6];

        printf("lan monitor thread() / IPSRC : %d / IPADDR : %d / NETMASK : %d\n", this->lanConfigChanged[LAN_PARAM_IP_SRC], this->lanConfigChanged[LAN_PARAM_IP_ADDR], this->lanConfigChanged[LAN_PARAM_NET_MASK]);

        get_ip_info(this->chan_index, b_ip, b_netmask, b_gw, b_mac); // get current network configuration
        ...
        if(this->lanConfigChanged[LAN_PARAM_IP_SRC] == 1 && this->ip_src == 1)
        {
            ...
            sprintf(str_net, "%d.%d.%d.%d", t_netmask[0], t_netmask[1], t_netmask[2], t_netmask[3]);
            ...
            if(this->lanConfigChanged[LAN_PARAM_MAC_ADDR] == 1)
            {
                ...
                sprintf(command, "ifconfig eth%d hw ether %02X:%02X:%02X:%02X:%02X:%02X", this->chan_index,
                ...
                sprintf(command, "route add default gw %d.%d.%d.%d eth%d", this->df_gw_ip_addr[0], this->df_gw_ip_addr[1], this->df_gw_ip_addr[2], this->df_gw_ip_addr[3], this->chan_index);
                system(command);
            }
        }
        else if(this->lanConfigChanged[LAN_PARAM_IP_SRC] == 1 && this->ip_src == 2)
        {
            for(int i = 0 ; i < 4 ; i++)
            {
                if(i != this->chan_index)
...
{
    uint8_t r_chan = 0;
    uint8_t ipAddr[4] = {0,}, netMask[4] = {0,}, macAddr[6] = {0,}, gwAddr[4] = {0,};
    ...
    dest_addr[0].insert(dest_addr[0].end(), std::begin(alert_addr), std::end(alert_addr));
    this->community_str = "KETI";

    this->lanConfigChanged.insert(std::make_pair<uint8_t, uint8_t>(LAN_PARAM_IP_SRC, 0));
    this->lanConfigChanged.insert(std::make_pair<uint8_t, uint8_t>(LAN_PARAM_IP_ADDR, 0));
    this->lanConfigChanged.insert(std::make_pair<uint8_t, uint8_t>(LAN_PARAM_MAC_ADDR, 0));
    this->lanConfigChanged.insert(std::make_pair<uint8_t, uint8_t>(LAN_PARAM_NET_MASK, 0));
    this->lanConfigChanged.insert(std::make_pair<uint8_t, uint8_t>(LAN_PARAM_IP_SRC, 0));
...
int get_ipv6_info(unsigned char chan, unsigned char *ipv6_str_out, unsigned char *prefix_out, unsigned char *ipv6_gw_out){
	...
	if(chan == 1){
		...
		if(op != NULL){
			...
			pclose(op);
			if(strlen(text) != 0){
				sprintf(cmds, "ifconfig %s | grep inet6 | awk \'{print $3}\' | awk -F \'%c\' \'{print $1}\'", DEV_NAME_SHARED, tmps);
						FILE *pp = popen(cmds, "r");

				sprintf(cmds, "route -A inet6 | grep %s | awk \'{print $1}\'", DEV_NAME_SHARED);
						FILE *gp = popen(cmds, "r");

				sprintf(cmds, "ifconfig %s | grep inet6 | awk \'{print $3}\' | awk -F \'%c\' \'{print $2}\' | awk -F \'/\' \'{print $2}\'", DEV_NAME_SHARED, tmps);
                ...
                buff[strlen(buff)] = '\0';
				memcpy((char *)prefix_out, buff, sizeof(unsigned char)*SIZE_NET_MASK_V6);
		...
		if(op != NULL){
			...
			if(strlen(text) != 0){
				...
				sprintf(cmds, "ifconfig %s | grep inet6 | awk \'{print $3}\' | awk -F \'%c\' \'{print $1}\'", DEV_NAME_DEDI, tmps);
						FILE *pp = popen(cmds, "r");

				sprintf(cmds, "route -A inet6 | grep %s | awk \'{print $1}\'", DEV_NAME_DEDI);
						FILE *gp = popen(cmds, "r");

				sprintf(cmds, "ifconfig %s | grep inet6 | awk \'{print $3}\' | awk -F \'%c\' \'{print $2}\' | awk -F \'/\' \'{print $2}\'", DEV_NAME_DEDI, tmps);
						FILE *cp = popen(cmds, "r");
...
// 	return policy_set;
// }
int Ipminetwork::send_alert_snmp(struct pef_policy_entry entry, const string alert_string)
{   
...
// 	uint8_t cmd[128];
// 	uint8_t ip[13];
// 	memcpy(ip, g_lan_config[chan].dest_addr[alert_num]+1, sizeof(uint8_t)*4);
	
// 	sprintf(cmd, "snmptrap -v 2c -Ci -c %s %d.%d.%d.%d \"\" ucdStart sysContact.0 s \"%s\"", g_lan_config[chan].community_str, 
// 			ip[0], ip[1], ip[2], ip[3], alert_string);
// 	int ret = system(cmd);
// 	if (ret != 0) {
