...
uint8_t sdr_en, fru_en, sel_en, ipmi_en, network_en, ntp_en, snmp_en, ssh_en,
    kvm_en, auth_en, web_en = 0;
...
static int parse_query(char *input, char *token, char *output) {
  char *query = strdup(input), *tokens = query, *p = query;
...
static void send_result_message(struct mg_connection *nc, uint8_t flag) {
  ...
  if (flag == 0) {
    ...
    memset(response, 0, sizeof(response));
    sprintf(response,
            "{\"CODE\":\"400\",\"MESSAGE\":\"FAIL\", \"DEBUG_CODE\":\"-1\"}");
    sprintf(header, "%sContent-Length: %d\r\n\r\n", ACCESS_RESPONSE_HEADER,
            strlen(response) + 2);
    mg_send_response_line(nc, 200, header);
    mg_send(nc, response, strlen(response) + 2);
    ...
    memset(response, 0, sizeof(response));
    sprintf(response,
            "{\"CODE\":\"400\",\"MESSAGE\":\"FAIL\", \"DEBUG_CODE\":\"-2\"}");
    sprintf(header, "%sContent-Length: %d\r\n\r\n", ACCESS_RESPONSE_HEADER,
            strlen(response) + 2);
    mg_send_response_line(nc, 200, header);
    mg_send(nc, response, strlen(response) + 2);
    ...
    memset(response, 0, sizeof(response));
    sprintf(response,
            "{\"CODE\":\"400\",\"MESSAGE\":\"FAIL\", \"DEBUG_CODE\":\"-3\"}");
    sprintf(header, "%sContent-Length: %d\r\n\r\n", ACCESS_RESPONSE_HEADER,
            strlen(response) + 2);
    mg_send_response_line(nc, 200, header);
    mg_send(nc, response, strlen(response) + 2);
...
static void handle_power_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    nc->flags |= MG_F_SEND_AND_CLOSE;
  } else if ((strcmp(methods, "POST") == 0) || (strcmp(methods, "PUT") == 0) ||
             (strcmp(methods, "OPTIONS") == 0)) {
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (strcmp(methods, "PUT") == 0) {
      if (b_len != 0) {
        ...
        set_fru_board(id, years, month, days, hours, minutes, secs, mfg.c_str(),
                      product.c_str(), serial.c_str(), part_num.c_str());

        set_fru_product(id, p_name.c_str(), p_mfg.c_str(), p_version.c_str(),
                        p_serial.c_str(), p_part_num.c_str());

        uint8_t loop = 0;
        for (loop = 0; loop < 30; loop++) {
          if (strncmp(c_type.c_str(), chassis_type_desc[loop],
                      strlen(c_type.c_str())) == 0)
            break;
        }
 ...
 * @author doyoung
 */
static void handle_sensor_call(struct mg_connection *nc, int ev,
                               void *ev_data) {
  ...
  } else if ((strcmp(methods, "OPTIONS") == 0) ||
             (strcmp(methods, "PUT") == 0)) {
    if (b_len == 0 && strcmp(methods, "OPTIONS") == 0) {
      mg_send_response_line(nc, 200, ACCESS_HEADER);
      mg_send(nc, "{}", 2);
      mg_send_http_chunk(nc, "", 0);
...
static void handle_ddns_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (q_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      uint8_t new_host_name[100], new_domain_name[300], new_ipv4_preferred[30],
          new_ipv4_alter[30], new_ipv6_preferred[100];
      ...
      if ((strcmp(new_host_name, "-") != 0) &&
          (strcmp(org_host_name.c_str(), new_host_name) != 0)) {
        int rets = 0;
      ...
      if ((strcmp(new_domain_name, "-") != 0) &&
          (strcmp(org_domain_name.c_str(), new_domain_name) != 0)) {
      ...
      if ((strcmp(new_ipv4_preferred, "-") != 0) &&
          (strcmp(org_ipv4_preferred.c_str(), new_ipv4_preferred) != 0)) {
      ...
      if ((strcmp(new_ipv4_alter, "-") != 0) &&
          (strcmp(org_ipv4_alter.c_str(), new_ipv4_alter) != 0)) {
      ...
      if ((strcmp(new_ipv6_preferred, "-") != 0) &&
          (strcmp(org_ipv6_preferred.c_str(), new_ipv6_preferred) != 0)) {
...
static void handle_network_call(struct mg_connection *nc, int ev,
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (q_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      uint8_t new_network_priority[10], new_lan_setting_enable[10],
          new_lan_interface[100], new_mac_address[100], new_ipv4_preferred[10],
          new_ipv4_dhcp_enable[10], new_ipv4_address[100],
          new_ipv4_netmask[100], new_ipv4_gateway[100],
          new_ipv6_subnet_prefix_length[100], new_ipv6_address[100],
          new_ipv6_enable[10], new_ipv6_dhcp_enable[10], new_ipv6_gateway[100],
          new_vlan_settings_enable[100], new_vlan_id[100],
          new_vlan_priority[100];
      ...
      parse_query(data, "IPV6_SUBNET_PREFIX_LENGTH",
                  new_ipv6_subnet_prefix_length);
      ...
      if (!strcmp(new_ipv4_dhcp_enable, "0")) {
        if (strcmp(org_ipv4_address.c_str(), new_ipv4_address) ||
          ...
          rets +=
              set_network_ipv4_ip(new_intf, new_ipv4_address, new_ipv4_netmask);
...
static void handle_ntp_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (q_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      int res_len = 0;
      uint8_t header[1000];
      uint8_t auto_sync[2], ntp_server[100], year[100], month[100], day[100],
          hour[100], min[100], sec[100];
      string sdata((char *)data);
      ...
      if (!strcmp(auto_sync, "1")) {
        ...
        system(cmds);
        sprintf(cmds2, "date --set=\"%s-%s-%s %s:%s:%s\"", year, month, day,
                hour, min, sec);
        int rets = 0;
        rets = system(cmds2);
        if (rets == 0) {
...
static void handle_smtp_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (q_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      uint8_t sender_address[100] =
          {
              ...
              primary_server_address[100] =
                  {
                      0,
                  },
              primary_user_name[100] =
                  {
                      0,
                  },
              primary_user_password[100] =
                  {
                      0,
                  },
              secondary_server_address[100] =
                  {
                      0,
                  },
              secondary_user_name[100] =
                  {
                      0,
                  },
              secondary_user_password[100] = {
                  0,
              };
      ...
      set_smtp_primary(primary_server_address, primary_user_name,
                       primary_user_password);
      set_smtp_secondary(secondary_server_address, secondary_user_name,
...
static void handle_ssl_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (q_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      uint8_t key_length[100] =
          {
              ...
              state_or_province[100] =
                  {
                      0,
                  },
              city_or_locality[100] =
                  {
                      0,
                  },
              organization[100] =
      ...
      set_ssl_info_1(key_length, country, state_or_province, city_or_locality,
                     organization, valid_for);
 ...
 * @author doyoung
 */
static void handle_radius_call(struct mg_connection *nc, int ev,
                               void *ev_data) {
...
static void handle_kvm_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  if (b_len != 0) {
    ...
    data[b_len] = 0;
             ...
             (strcmp(methods, "PUT") == 0) || (strcmp(methods, "POST") == 0)) {
    if (b_len == 0 && strcmp(methods, "OPTIONS") == 0) {
      mg_send_response_line(nc, 200, ACCESS_HEADER);
      mg_send(nc, "{}", 2);
      mg_send_http_chunk(nc, "", 0);
  ...
  if ((strcmp(methods, "POST") == 0) || (strcmp(methods, "OPTIONS") == 0)) {
    if (b_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      sprintf(header, "%sContent-Length: %d\r\n\r\n", ACCESS_RESPONSE_HEADER,
              strlen(res_msg));
      mg_send_response_line(nc, 200, header);
      cout << "\t res_msg :" << res_msg << " res_msg size :" << strlen(res_msg)
           << endl;
...
static void handle_user_call(struct mg_connection *nc, int ev, void *ev_data) {
  ...
  string name, index, password, enable_status, callback, linkauth, ipmimsg,
      privilege;
  ...
  if (strcmp(methods, "GET") == 0) {
    ...
    if (b_len == 0 && (strcmp(methods, "OPTIONS") == 0)) {
      ...
      if (stoi(index) <= 10) {
        ...
        rets += set_user_access(index.c_str(), enable_status.c_str(),
                                callback.c_str(), linkauth.c_str(),
                                ipmimsg.c_str(), privilege.c_str());
        if (rets == 0)
          send_result_message(nc, 1);
        else
          send_result_message(nc, 0);
...
static void handle_main_call(struct mg_connection *nc, int ev,
...
void restful_init(void) {
  ...
  nc = mg_bind(
      &mgr, s_http_port,
      ev_handler); // mg_bind_opt(&mgr, s_http_port, ev_handler, bind_opts);
