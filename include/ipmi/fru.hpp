...
// /**
//  * @brief 
//  * storage_get_fruid_data 요청 시 클라이언트 ipmitool로 전송할 메세지의 최대 길이를 정의합니다.\n
//  * ipmitool fru edit 명령어 실행 시 request message의 4번째 Byte 데이터는 FRU Data의 길이를 나타냅니다.\n
//  * 만약 FRU Data length가 MAX_READ_SIZE와 동일할 경우 edit_flag를 1로 바꿉니다.\n
//  */ 
...
void rest_set_fru_board(int id, char f_mfg_date[4], char f_mfg[LEN_MFG], char f_product[LEN_PRODUCT], char f_serial[LEN_SERIAL], char f_part_num[LEN_PART_NUM]);
void rest_set_fru_product(int id, char f_name[LEN_NAME], char f_mfg[LEN_MFG], char f_version[LEN_VERSION], char f_serial[LEN_SERIAL], char f_part_num[LEN_PART_NUM]);
...
class Ipmifru
{
	...
	/// @brief storage_get_fruid_data 함수 호출 시 클라이언트로 FRU Inventory Data 및 header 정보를 보낼 때 보낸 데이터의 길이를 저장하는 변수
...
/// @brief plat_fru_device_init 함수 호출 시 chassis, board, product 정보가 있을 경우 flag를 1로 설정
unsigned char chassis_flag, board_flag, product_flag= {0};

/**
 * @brief ipmitool  fru edit 명령어 실행 시 수정된 FRU Inventory Data 를 저장하는 문자열 변수\n
 * ipmitool fru edit 명령어 실행 시 FRU Inventory Data 중 수정할 부분만 새로 수정된 데이터가 Request Message로 전송됩니다.\n
 * Example) <FRU Header><FRU Chassis><FRU Board><FRU Product> 와 같은 배열로 되어있으며, Board 정보를 수정할 경우 <FRU Board>만 교체되어 전체 메세지가 전송됩니다.
 * 
 */
