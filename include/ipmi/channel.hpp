...
class Ipmichannel
{
    ...
    public:
        Ipmichannel(uint8_t _c_num) : medium_type(0), protocol_type(0), channel_number(_c_num)
        {
        }
        Ipmichannel(uint8_t _m_type, uint8_t _p_type, uint8_t _c_num, uint8_t _s_info) : medium_type(_m_type), protocol_type(_p_type), channel_number(_c_num), session_info(_s_info)
        {
            access_mode = 0;
            alerting = 0;
            priv_limit = 0;
