...
class Ipmiapplication
{
    ...
    public:
        ...
        void app_get_sysinfo_param(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_set_sysinfo_param(uint8_t *request, uint8_t *response, uint8_t *res_len);
        
        void app_set_user_passwd(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_set_useraccess(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_set_username(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_get_useraccess(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_get_username(uint8_t *request, uint8_t *response, uint8_t *res_len);

        void app_get_session_info(uint8_t *request, uint8_t *response, uint8_t *res_len);

        void app_bmc_selftest(uint8_t *response, uint8_t *res_len);
        void app_clear_message_flag(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_cold_reset(uint8_t *response, uint8_t *res_len);
        void app_warm_reset(uint8_t *response, uint8_t *res_len);
        void app_set_global_enables(uint8_t *request, uint8_t *response, uint8_t *res_len);
        ...
        void app_set_session_priv(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_set_channel_access(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_get_channel_access(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_get_channel_info(uint8_t *request, uint8_t *response, uint8_t *res_len);
        void app_get_channel_auth_cap(uint8_t *request, uint8_t *response, uint8_t *res_len);
 ...
 * 
 * @param unsigned char * build_time - BMC Build time 버전 정보를 반환받을 문자열 포인터
 * @return NULL
 */
...
/** @brief flag에 따라 REST 요청 처리, Primary receiver, Secondary receiver 주소를 반환하는 함수\n
 * 
 * 본 함수는 입력된 flag에 따라 REST 요청을 처리하거나 Primary / Secondary receiver Address를 반환합니다.\n
 * 
 ...
 * 
 * 본 함수는 SMTP 설정정보를 SMTP Config파일인 .msmtprc 파일에 저장하는 함수입니다.\n
...
* 파일이 존재하지 않거나 내용이 없을 경우 빈 내용을 JSON 문자열 형태로 생성하여 반환\n
* 저장된 파일 정보는 KETI-Web 에서 클라이언트로부터 SSL 인증서 정보 요청 시 시각화 하여 보여줌\n
...
* 본 함수는 SSL 인증서 정보를 파일에 저장하는 함수로 입력된 매개변수로부터 정보를 받아 SSL_BIN 파일에 저장\n
* 저장된 파일 정보는 KETI-Web 에서 클라이언트로부터 SSL 인증서 정보 요청 시 JSON 문자열 형태로 반환.\n
...
* 본 함수는 localtime 함수를 사용하여 현재 날짜로부터 인증서 만료 날짜를 생성하여 문자열로 반환\n
* @param int days - 만료일\n
...
* 본 함수는 Active Directory 및 LDAP 설정 정보 요청 시 JSON 문자열 형태로 생성하여 반환하는 함수이다.\n 
* get_ad_ldap_info 함수를 호출하여 설정 정보를 불러와 JSON 문자열로 생성한다.\n
...
* 본 함수는 Active Directory 설정을 반영할 때 Active Directory를 활성화 / 비활성화 하는 함수이다.\n 
* 본 함수를 호출하면 LDAP, RADIUS 기능은 비활성화된다 (KETI-REST Function Call에 의해 반영)\n
* @param int enable - Active Directory 활성화 (1) / 비활성화(0)\n
* @return 실패시 0\n
*/
int set_ad_enable(int enable);
/**
* @brief Active Directory 설정 정보 (AD 서버 IP, 비밀번호)를 파일에 저장하는 함수\n
* 본 함수는 Active Directory 설정 정보를 파일에 저장하는 함수이다.\n
* 입력된 설정 정보는 AD_BIN 파일에 저장되며 클라이언트가 AD 설정정보 요청 시 해당 파일에서 정보를 불러와 반환한다.\n
...
/**
* @brief Active Directory 설정 정보 (Active Directory Domain)를 파일에 저장하는 함수\n
* 본 함수는 Active Directory 설정 정보를 파일에 저장하는 함수이다.\n
* 입력된 설정 정보는 AD_BIN 파일에 저장되며 클라이언트가 AD 설정정보 요청 시 해당 파일에서 정보를 불러와 반환한다.\n
...
/**
* @brief Active Directory 설정 정보 (Active Directory 서버 사용자 계정)를 파일에 저장하는 함수\n
* 본 함수는 Active Directory 설정 정보를 파일에 저장하는 함수이다.\n
* 입력된 설정 정보는 AD_BIN 파일에 저장되며 클라이언트가 AD 설정정보 요청 시 해당 파일에서 정보를 불러와 반환한다.\n
...
* @brief Active Directory 설정 정보 (Active Directory 서버 사용자 계정)를 파일에 저장하는 함수\n
* 본 함수는 Active Directory 설정 정보를 파일에 저장하는 함수이다.\n
* 입력된 설정 정보는 AD_BIN 파일에 저장되며 전체 설정정보를 모아 /etc/nslcd.conf 파일에 반영한다.\n
* 설정이 저장되면 KETI-Web 로그인 시 Active Directory 서버 인증을 통하여 로그인한다.\n
* @param void\n
* @return 실패시 0\n
*/
...
* LDAP 서버 사용 시, AD 서버 인증 사용 시 호출되는 함수로 LDAP 서버 사용 유무를 활성화 / 비활성화 한다.\n
* 설정 정보는 LDAP_BIN 파일에 저장되며, 추후 LDAP 정보 요청 시 해당 정보를 반영하여 불러온다.\n
...
* 입력된 설정 정보는 LDAP_BIN 파일에 저장되며 클라이언트가 LDAP 설정정보 요청 시 해당 파일에서 정보를 불러와 반환한다.\n
...
/**
* @brief LDAP 설정 정보 (LDAP 인증 서버 Searchbase - Base Domain name)를 파일에 저장하는 함수\n
* 본 함수는 LDAP 설정 정보를 파일에 저장하는 함수이다.\n
* 입력된 설정 정보는 LDAP_BIN 파일에 저장되며 클라이언트가 LDAP 설정정보 요청 시 해당 파일에서 정보를 불러와 반환한다.\n
...
/**
* @brief LDAP 설정 정보 (LDAP 인증 서버 Bind Domain name)를 파일에 저장하는 함수\n
* 본 함수는 LDAP 설정 정보를 파일에 저장하는 함수이다.\n
* 입력된 설정 정보는 LDAP_BIN 파일에 저장되며 클라이언트가 LDAP 설정정보 요청 시 해당 파일에서 정보를 불러와 반환한다.\n
